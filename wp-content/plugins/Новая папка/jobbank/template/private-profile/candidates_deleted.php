							...
							<div class="col-md-12">
								...
								<div class="group-button mt-3">
									<?php
										...
										<a target="_blank" href="<?php echo esc_url(get_post_meta($candidate_post_id,'cv_file_url',true)); ?>" class="btn btn-small-ar mb-2" data-toggle="tooltip" title="<?php esc_html_e( 'Print PDF', 'jobbank' );?>"> <i class="far fa-file-alt"></i></a>
										<?php
										}
									?>
									<span id="shortlistdeleted<?php echo esc_attr($candidate_post_id); ?>">
										<?php
											...
											?>
											<button id="" class="btn btn-small-ar mb-2 <?php echo (get_post_meta($candidate_post_id, 'jobbank_candidate_shortlisted', true)=='yes'?' shortlisted':''); ?>" data-toggle="tooltip"  onclick="jobbank_candidate_shortlisted_delete('<?php echo esc_attr($candidate_post_id);?>','deleted')" title="<?php esc_html_e( 'Short Listed', 'jobbank' );?>" ><i class="fas fa-user-check mr-1"></i><?php esc_html_e( 'Shortlist', 'jobbank' );?></button>
											<?php
												}else{
											?>
											<button id="" class="btn btn-small-ar mb-2 <?php echo (get_post_meta($candidate_post_id, 'jobbank_candidate_shortlisted', true)=='yes'?' shortlisted':''); ?>" data-toggle="tooltip"  onclick="jobbank_candidate_shortlisted('<?php echo esc_attr($candidate_post_id);?>','deleted')" title="<?php esc_html_e( 'Make Short Listed', 'jobbank' );?>" ><i class="fas fa-user-check mr-1"></i><?php esc_html_e( 'Shortlist', 'jobbank' );?></button>
											<?php
											}
										?>
									</span>
									<button id="scheduledeletedbutton<?php echo esc_attr($candidate_post_id);?>"class="btn btn-small-ar mb-2 <?php echo (get_post_meta($candidate_post_id, 'candidate_schedule', true)=='yes'?' shortlisted':''); ?>" onclick="jobbank_candidate_meeting_popup('<?php echo esc_attr($candidate_post_id);?>')" data-toggle="tooltip" title="<?php esc_html_e( 'Meeting Schedule', 'jobbank' );?>" ><i class="fas fa-user-clock mr-1"></i><?php esc_html_e( 'Meeting', 'jobbank' );?></button>
									...
									<span id="rejectlist<?php echo esc_attr($candidate_post_id); ?>">
										<?php
											...
											?>
											<button class="btn btn-small-ar mb-2 btn-delete  shortlisted" onclick="jobbank_candidate_reject_delete('<?php echo esc_attr($candidate_post_id);?>','list')" data-toggle="tooltip" title="<?php esc_html_e( 'Undo Reject', 'jobbank' );?>"><i class="fas fa-user-times mr-1"></i><?php esc_html_e( 'Reject', 'jobbank' );?></button>
											<?php
												}else{
											?>
											<button class="btn btn-small-ar mb-2 btn-delete " onclick="jobbank_candidate_reject('<?php echo esc_attr($candidate_post_id);?>','list')" data-toggle="tooltip" title="<?php esc_html_e( 'Make Reject', 'jobbank' );?>"><i class="fas fa-user-times mr-1"></i><?php esc_html_e( 'Reject', 'jobbank' );?></button>
											<?php
											}
										?>
									</span>	
